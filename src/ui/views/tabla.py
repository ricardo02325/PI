...
def obtener_datos_db():
    ...
    conexion = mysql.connector.connect(
        ...
        user=DB_CONFIG["root"],
        password=DB_CONFIG[""],
        database=DB_CONFIG["sistema_hidroponico"],
    )

    cursor = conexion.cursor()
    query = "SELECT * FROM inf_sensores WHERE id_lectura = ( SELECT MAX(id_lectura) FROM inf_sensores AS sub WHERE sub.id_sensor = inf_sensores.id_sensor );"
