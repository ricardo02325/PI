...
ctk.set_appearance_mode("dark")  # Modo oscuro para mejor apariencia
ctk.set_default_color_theme("blue")  # Tema azul
style.use('ggplot')

# Crear la ventana principal
root = ctk.CTk()
root.title("Sistema Hidropónico - Sensores")
...
# Configuración de la figura de Matplotlib con tres subgráficos en una sola fila
...
def actualizar_graficas(frame):
    ...
    # Configurar los títulos y etiquetas siempre visibles
    ...
    if df is None:
        for ax in [ax1, ax2, ax3]:
            ...
            ax.set_xlim(0, 1)  # Limitar el eje X temporalmente
            ax.set_ylim(0, 1)  # Limitar el eje Y para evitar escalados raros
        canvas.draw()
        return
    ...
    if not df_ph.empty:
        ...
        ax1.set_ylim(0, 14)  # Rango típico del pH
    ...
    if not df_ce.empty:
        ...
        ax2.set_ylim(0, 10)  # Rango arbitrario para evitar gráficos vacíos
    ...
    if not df_temp.empty:
        ...
        ax3.set_ylim(0, 50)  # Rango de temperatura para evitar gráficos vacíos
